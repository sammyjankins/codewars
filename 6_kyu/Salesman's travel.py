...
# The list of clients to visit was given as a string of all addresses, each separated from the others by a comma,
# e.g. :
#
...
def travel(r, zipcode):
    ...
    for address in r.split(','):
        if address[-8:] == zipcode:
            buf = address[:-8].split()
            houses.append(buf.pop(0))
            street_and_town.append(' '.join(buf))
    return '{}:{}/{}'.format(zipcode, ','.join(street_and_town), ','.join(houses))
